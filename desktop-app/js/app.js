class App {
    ...
    showStatus(message, isWarning, showCopy) {
        ...
        this.statusbar.className = isWarning
            ? 'status-bar-warning'
            : 'status-bar';
    ...
    openBackups() {
        this.getMyBackups().then(async files => {
            ...
            backupsContainer.innerHTML = files.length
                ...
                : '<br><h5>&nbsp;&nbsp;&nbsp;No Backups made!! Make a backup now!</h5>';
            if (
                (await this.bsaber.hasBeatSDFolder('data')) ||
                (await this.bsaber.hasBeatSDFolder('obb'))
            ) {
...
<span class="link" data-url="https://bsaber.com/members/elliotttate/">@elliotttate</span> for beat saber efforts.`;
        ...
        if (isBeatSaberInstalled && isQuestSaberPatch) {
            ...
            songs.forEach(song => {
                ...
                child.querySelector(
                    '.image'
                ).style.backgroundColor = randomColor({ seed: song.id });
            ...
            this.container.innerHTML =
                '<h4>Still downloading files plaese wait and try again...</h4>';
        }
    }
    openDeviceSettings() {
        this.add_repo.style.display = 'none';
