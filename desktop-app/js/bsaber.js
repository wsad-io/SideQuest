class Bsaber {
    ...
    signAPK(dir) {
        return new Promise(resolve => {
            ...
            let callback = data => {
                document.getElementById('patch-loading-message').innerHTML =
                    'Installing now,<br>' +
                    (data.toString().length
                        ? '<span class="chip">' + data.toString() + '</span>'
                        : '');
            };
            ls.stdout.on('data', callback);
            ls.stderr.on('data', callback);
    ...
    downloadAppSigner() {
        let url =
            'https://github.com/patrickfav/uber-apk-signer/releases/download/v1.0.0/uber-apk-signer-1.0.0.jar';
        ...
        return new Promise((resolve, reject) => {
            this.app.setup
                .downloadFile(url, url, url, () => downloadPath)
                .then(path => {
    ...
    downloadQuestSaberPatch() {
        let url =
            'https://github.com/trishume/QuestSaberPatch/releases/download/v0.4.0/questsaberpatch-';
        let name = 'app.exe';
        switch (os.platform()) {
            case 'win32':
                ...
                transfer =>
                    new Promise((resolve, reject) => {
                        ...
                        transfer.on('progress', stats => {
                            this.app.spinner_loading_message.innerText =
                                'Backing Up - ' +
                                Math.round(
