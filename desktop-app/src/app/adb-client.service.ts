...
export class AdbClientService {
  ...
  constructor(public appService:AppService,private spinnerService:LoadingSpinnerService,private statusService:StatusBarService) {
    ...
    switch (status) {
      case ConnectionStatus.TOOMANY:
        this.deviceStatusMessage =
          'Warning: Please connect only one android device to your PC';
        break;
  ...
  async downloadFile(winUrl, linUrl, macUrl, getPath) {
    ...
    return new Promise((resolve, reject) => {
      this.appService.progress(this.appService.request(downloadUrl, requestOptions), { throttle: 300 })
        ...
        .pipe(this.appService.fs.createWriteStream(downloadPath));
    });
  }
  getUserAgent() {
    const nodeString = `NodeJs/${(<any>window).process.version}`;
    const packageString = 'OpenStoreVR';
    const computerString = `Hostname/${this.appService.os.hostname()} Platform/${this.appService.os.platform()} PlatformVersion/${this.appService.os.release()}`;
  ...
  backupPackage(apkPath, packageName) {
    ...
    return this.client
      .pull(this.deviceSerial, apkPath)
      .then(
        transfer =>
  ...
  installLocalApk(filepath, dontCatchError) {
    let p = this.client.install(
      this.deviceSerial,
      this.appService.fs.createReadStream(filepath)
    );
    if (!dontCatchError) {
      p = p.catch(e => {
        this.spinnerService.hideLoader();
          ...
          let installableFiles = Object.keys(files).filter(
            (val, index) => {
              return Object.keys(typeBasedActions).includes(
                this.appService.path.extname(files[index])
              );
            }
          );
          installableFiles.forEach(file => {
            typeBasedActions[this.appService.path.extname(filepath)](
              filepath,
              this
            );
          });
    ...
    return this.client
      .install(
        ...
        new this.appService.Readable().wrap(
          this.appService.progress(this.appService.request(url), { throttle: 300 }).on(
            'progress',
            state => {
              this.spinnerService.setMessage(
                'Downloading APK... ' +
