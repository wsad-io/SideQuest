...
export class AdbClientService {
  ...
  getPackages() {
    ...
    return this.adbCommand('getPackages',{serial:this.deviceSerial})
    //return this.client.getPackages(this.deviceSerial)
      .then(packages => {
  ...
  getPackageInfo(packageName) {
    return this.adbCommand('shell',{serial:this.deviceSerial,command:'dumpsys package ' + packageName + ' | grep versionName'})
    // this.client
  ...
  async connectedStatus() {
    ...
    if (now - this.lastConnectionCheck < this.pollInterval || this.isTransferring) return requestAnimationFrame(this.connectedStatus.bind(this));
  ...
  getFilenameDate(){
    return JSON.stringify(new Date()).split('"').join('').split(':').join('-').trim()
  }
  getPackageLocation(packageName) {
    return this.adbCommand('shell',{serial:this.deviceSerial,command:'pm list packages -f ' + packageName})
      .then(res => {
        let parts = res.split(':');
        if (parts.length > 1) {
  ...
  async makeBackupFolders(packageName:string){
    ...
    return this.appService.mkdir(mainBackupPath)
  ...
  async getBackups(packageName:string){
    ...
    return this.appService.fs.readdirSync(backupPath)
      .map(file=>this.appService.path.join(backupPath,file))
      .filter(file =>
        !this.appService.fs.lstatSync(file).isDirectory() &&
        this.appService.path.extname(file) === '.apk'
      );
  }
  ...
  async backupPackage(apkPath, packageName) {
    ...
    return this.makeBackupFolders(packageName)
      .then(()=>this.adbCommand('pull',{serial:this.deviceSerial,path:apkPath,savePath:savePath},stats=>{
      ...
      .then(()=>{
        ...
        this.statusService.showStatus(packageName + ' backed up to '+ this.appService.path.basename(savePath) +' successfully!!');
        return savePath;
      })
      .catch(e=>{
        this.spinnerService.hideLoader();
  ...
  installAPK(filePath:string,isLocal?:boolean){
    ...
    return this.adbCommand('install',{serial:this.deviceSerial,path:filePath,isLocal:!!isLocal},status=>{
      if(status.done){
        ...
        this.spinnerService.setMessage('Downloading APK:<br>'+filePath+'<br>'+(Math.round(status.percent * 100))+'MB')
      }
    })
      .then(r=>{
        this.spinnerService.hideLoader();
  ...
  makeFolder(files){
    ...
    return this.appService.mkdir(f.saveName)
      .then(()=>this.makeFolder(files));
  ...
  downloadFile(files){
    ...
    return this.adbCommand('pull',{serial:this.deviceSerial,path:f.name,savePath:f.saveName},stats=>{
      this.spinnerService.setMessage('File downloading: ' + this.appService.path.basename(f.name) + ' <br>' +
        (Math.round(
      ...
      .then(entries=>{
        ...
        let recurse = ()=>{
          if(!folders.length) return null;
            ...
            return this.getFoldersRecursive(folder.name)
              .then(()=>recurse())
        };
        return recurse();
      })
    ...
    return new Promise(resolve=>{
      this.appService.fs.readdir(root,async(err,entries)=> {
        ...
        let recurse = ()=>{
          ...
          let folder = folders.shift();
          return this.getLocalFoldersRecursive(folder.name)
            .then(()=>recurse());
        };
        return recurse()
          .then(()=>resolve());
  ...
  async uploadFile(files){
    ...
    return this.adbCommand('push',{serial:this.deviceSerial,path:f.name,savePath:f.savePath},stats=>{
      this.spinnerService.setMessage('File uploading: ' + this.appService.path.basename(f.name) + ' <br>' +
        (Math.round(
          (stats.bytesTransferred / 1024 / 1024)*100
        )/100) +
        'MB');
  ...
  async restoreDataBackup(packageName:string,folderName:string){
    ...
    this.spinnerService.showLoader();
    let packageBackupPath = this.appService.path.join(this.appService.appData,
      'backups',packageName,'data',folderName,'files');
      ...
      await this.getLocalFoldersRecursive(packageBackupPath)
        .then(()=>{
          this.localFiles.forEach(file=>{
            file.savePath =
              this.appService.path.posix.join(
              '/sdcard/Android/data/'+packageName+'/files',
                file.name.replace(packageBackupPath,'').split('\\').join('/')
            );
          });
    ...
    let obbBackupPath = this.appService.path.join(this.appService.appData,
      'backups',packageName,'data',folderName,'obb');
  ...
  async makeDataBackup(packageName:string) {
    ...
    let packageBackupPath = this.appService.path.join(this.appService.appData,
      'backups',packageName,'data',folderName);
    this.files = [];
    await this.adbCommand('stat',{serial:this.deviceSerial,path:'/sdcard/Android/data/'+packageName})
      .then(()=>this.adbCommand('stat',{serial:this.deviceSerial,path:'/sdcard/Android/data/'+packageName+'/files/'}))
      .then(()=>{
        return this.getFoldersRecursive('/sdcard/Android/data/'+packageName+'/files/')
          ...
          .then(()=>this.files = this.files.map(f=> {
  ...
  async getFolders(root) {
    return this.adbCommand('readdir',{serial:this.deviceSerial,path:root})
