...
export class HeaderComponent implements OnInit {
  ...
  patchBeatSaber(){
    ...
    if (!~this.bsaberService.supportedBeatSaberVersions.indexOf(this.bsaberService.beatSaberVersion)) {
      return this.statusService.showStatus('Beat Saber version not supported yet!! '+this.bsaberService.beatSaberVersion, true)
    }
  ...
  getReplaceAndColors(){
    this.replaceText = Object.keys(this.bsaberService.jSon.replaceText).map(k=>({key:k,value:this.bsaberService.jSon.replaceText[k]}));
    if(this.bsaberService.jSon.colors.colorA){
      this.colorA = 'rgba('+
        (this.bsaberService.jSon.colors.colorA.r*255)+','+
        (this.bsaberService.jSon.colors.colorA.g*255)+','+
        (this.bsaberService.jSon.colors.colorA.b*255)+',1)';
    }else{
    ...
    if(this.bsaberService.jSon.colors.colorB){
      this.colorB = 'rgba('+
        ...
        (this.bsaberService.jSon.colors.colorB.b*255)+',1)';
    }else{
